...
public class ThirdPersonCharacter : MonoBehaviour {
	...
	[SerializeField] float minAirMomentum = 6;								// determines the max speed of the character while airborne
	[SerializeField] float airDecelerationRate = 1;								// determines the max speed of the character while airborne
	...
	[Range(0.1f,1)] [SerializeField] public float slomoMultiplier = 0.5f;	//Custom value for demo purposes. While you hold space, you spend more time in air.
	...
	public void Move (Vector3 move, bool crouch, bool jump, Vector3 lookPos) {
		...
		//Debug.Log("8 !!!!!!!!!!!~~~~~~~~~~~~~~~~ velocity: "+velocity+"; move: "+move);
	...
	{ 
		...
		if(Input.GetButton("Jump")){
			localGravityMultiplier *= slomoMultiplier;
			localAirControl *= slomoMultiplier;
		}
		...
		//Vector3 airMove = new Vector3 (Mathf.Max(moveInput.x,airMomentum) * airSpeed, velocity.y, Mathf.Max(moveInput.z,airMomentum) * airSpeed);
		//Vector3 airMove = new Vector3 ((moveInput.x+0.1f) * airSpeed *aAirMom, velocity.y, (moveInput.z+0.1f) * airSpeed *aAirMom);
		//Vector3 airMove = new Vector3 (moveInput.x* airSpeed, velocity.y, moveInput.z * airSpeed );
		Vector3 airMove = new Vector3 (moveInput.x* localAirControl, 0, moveInput.z * localAirControl );
		//TODO: this doesn't work until you handle it globally.
		//airMove = Vector3.Lerp (airMove, terminalAirControlVelocity, Time.deltaTime * airControlDecelerationRate);
		
		//aAirMom = Mathf.Lerp (aAirMom, 1, airSpeed/10*Time.deltaTime);
		/*
		jumpSpeedCurve = Vector3.Lerp (jumpSpeedCurve, terminalAirVelocity, Time.deltaTime * airMomentum);


		//velocity = Vector3.Lerp (velocity, airMove, Time.deltaTime * localAirControl);
		velocity = Vector3.Lerp (velocity, airMove, Time.deltaTime * localAirControl);
		if(Mathf.Abs(velocity.x)<Mathf.Abs(jumpSpeedCurve.x)){
			velocity.x = jumpSpeedCurve.x;
		}
		if(Mathf.Abs(velocity.z)<Mathf.Abs(jumpSpeedCurve.z)){
			velocity.z = jumpSpeedCurve.z;
		}
		//velocity = (velocity + jumpSpeedCurve);
		*/
		...
		/*
		if(terminalAirVelocity.x * airMove.x <0){
			terminalAirVelocity.x += airMove.x;
		}
		else if(Mathf.Abs(airMove.x)>Mathf.Abs(terminalAirVelocity.x)){
				terminalAirVelocity.x = airMove.x;
		}

		if(terminalAirVelocity.y * airMove.y <0){
			terminalAirVelocity.y += airMove.y;
		}
		else if(Mathf.Abs(airMove.y)>Mathf.Abs(terminalAirVelocity.y)){
			terminalAirVelocity.y = airMove.y;
		}
		*/
		/*
		//give more power to airMove if you're trying to go in the opposite direction of the current velocity
		if(velocity.x * airMove.x < 0 ){
			//airMove.x -= velocity.x/300;
			//airMove.x -= velocity.x*10;
			velocity.x += airMove.x/8;
		}
		if(velocity.z * airMove.z < 0 ){
			//airMove.z -= velocity.z/300;
			//airMove.z -= velocity.z*10;
			velocity.z += airMove.z/8;
		}
		*/
		
		/*
		Vector3 lerpTarget = terminalAirVelocity+airMove;
		if(lerpTarget.x > terminalAirSpeed)
			lerpTarget.x = terminalAirSpeed;
		else if(lerpTarget.x < -terminalAirSpeed)
			lerpTarget.x = -terminalAirSpeed;

		if(lerpTarget.z > terminalAirSpeed)
			lerpTarget.z = terminalAirSpeed;
		else if(lerpTarget.z < -terminalAirSpeed)
			lerpTarget.z = -terminalAirSpeed;
		*/
		...
		else if(velocity.x < -terminalAirSpeed)
			velocity.x = -terminalAirSpeed;
		
		if(velocity.z > terminalAirSpeed)
			velocity.z = terminalAirSpeed;
		else if(velocity.z < -terminalAirSpeed)
			velocity.z = -terminalAirSpeed;*/
		/*
		if(velocity.magnitude > terminalAirSpeed){
			float d = velocity.magnitude / terminalAirSpeed;
			velocity /= d;
		}
		*/
		if(velocity.x > terminalAirSpeed)
		...
		float antigravity = 0;
		if(jumpInput && rigidbody.velocity.y >jumpPower*0.9f){// 0.3f){
